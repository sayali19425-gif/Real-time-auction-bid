...
//-- global auction data --//
const auction = {
  item: "Stellar Genesis NFT #001",
  currentBid: 42.5,
  topBidder: "no bid yet",
  endTime: (() => {
    let end = sessionStorage.getItem("end");
    if (!end) { end = Date.now() + 24 * 60 * 60 * 1000; sessionStorage.setItem("end", end); }
    return Number(end);
  })(),
};
...
function Page1({ onConnected }) {
  ...
  function handleConnect() {
    ...
    if (!key) return setStatus({ ok: false, msg: "Please enter your Stellar wallet address." });
    if (!key.startsWith("G") || key.length < 56) return setStatus({ ok: false, msg: "Invalid Address. Must start with G and be 56 characters." });
    setStatus({ ok: true, msg: "✅ Wallet connected successfully!" });
    setTimeout(() => onConnected(key), 1200);
  }
...
function Page2({ wallet, onGoHistory }) {
  ...
  useEffect(() => { const id = setInterval(() => setData({ ...auction }), 3000); return () => clearInterval(id); }, []);
  ...
  async function placeBid(address, amount, label) {
    ...
    if (val <= auction.currentBid) return setStatus({ ok: false, msg: `❌ ${label} failed - ${val} XLM is less than the current bid of ${auction.currentBid} XLM.` });

    setBusy(true); setStatus(null);
    await new Promise((r) => setTimeout(r, 1500));

    auction.currentBid = val;
    auction.topBidder = address;
